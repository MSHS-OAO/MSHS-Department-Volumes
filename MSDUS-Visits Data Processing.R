...
path_dictionary_Premier_volume <- choose.files(caption = 'Select DUS Main Dictionary',multi = F)
dictionary_Premier_volume <- read_xlsx(path_dictionary_Premier_volume, sheet = 'VolumeID to Cost center # Map', col_types = c('guess', 'text'), col_names = c('Volume ID', 'Cost Center'), skip = 1)

#eIDX/IDX Dictionaries
  # Rollup + Department Map for Visits:
  dictionary_eIDX_rollup_department <- read_xlsx(path_dictionary_Premier_volume, sheet = 'Sch Loc to Dept Map visit e.IDX', col_types = c('skip', 'skip', 'text', 'text', 'text','skip', 'skip', 'skip'))
  # Volume ID (Premier),Type, and eIDX/idx Department Map:
  dictionary_eIDX_departments <- read_xlsx(path_dictionary_Premier_volume, sheet = 'New eIDX visit - volID map', col_types = c('text', 'text', 'text', 'skip'))
  ...
  remove_departments_eIDX <- read_xlsx(path_dictionary_Premier_volume, sheet = 'eIDX_IDX Departments to Remove')
#Epic Dictionaries
  #Importing the Dictionaries
  dictionary_Epic_department_VolID <- read_xlsx(path_dictionary_Premier_volume, sheet = 'New Epic Volume ID Map',col_types = c('text', 'text', 'skip'))
...
#Removing Departments
  ...
  data_Epic_merge <- data_Epic[!data_Epic$Department %in% remove_departments_Epic,]
#Date Range Check 
  #eIDX/IDX
    choices_date_range_eIDX <- format(unique(data_eIDXIDX_visits$`SchDateId Date (MM/DD/YYYY)`), "%m/%d/%Y")
    remove_dates_eIDX <- select.list(choices = c('None', choices_date_range_eIDX), title = "eIDX/IDX:Remove Dates?", multiple = T, graphics = T, preselect ='None' )
    if (remove_dates_eIDX != 'None' | is.na(remove_dates_eIDX)) {
      remove_dates_eIDX<- as.Date(remove_dates_eIDX, tryFormats = "%m/%d/%Y")
      data_eIDXIDX_merge<- data_eIDXIDX_visits[!(data_eIDXIDX_visits$`SchDateId Date (MM/DD/YYYY)` %in% remove_dates_eIDX),]
    } #remove dates if user chooses
  #Epic
    choices_date_range_Epic <- format(unique(data_Epic$`Appt Date`), "%m/%d/%Y")
    remove_dates_Epic <- select.list(choices = c('None', choices_date_range_Epic), title = "Epic:Remove Dates?", multiple = T, graphics = T, preselect ='None' )
...
#Selecting only needed columns
    ...
    data_Epic_merge$Volume <- rep(1 , length(data_Epic_merge$`Cost Center`))
...
#Aggregating Data
  data_visits <- aggregate(data_visits$Volume, by = list(data_visits$`Entity ID`, data_visits$`Facility ID`, data_visits$`Cost Center`, data_visits$`Start Date`, data_visits$`End Date`, data_visits$`Volume ID`, data_visits$Budget), FUN = 'sum' )
...
#Formatting Data
  ...
  data_visits$`Start Date` <- format(data_visits$`Start Date`, "%m/%d/%Y")
